...
pub fn printParseError(allocator: Allocator, writer: Writer, lex: Lexer, token: Token, src_file: []const u8) !void {
    var lexer = lex;
    const data = token.data;
    const info = token.info;
    // Print source file with line and position
    try format.updateStyle(writer, .{ .font_style = .{ .bold = true } }, null);
    const src_msg = try std.fmt.allocPrint(allocator, "{s}:{d}:{d}: ", .{ src_file, info.line, info.pos });
    ...
    const source_aligned = try std.fmt.allocPrint(allocator, "  {s}", .{line_source});
    defer allocator.free(source_aligned);
    try writer.writeAll(source_aligned);
    // Print a pointer to where the error occured
    const ptr_msg = try allocator.alloc(u8, 2 + line_source.len + 1);
